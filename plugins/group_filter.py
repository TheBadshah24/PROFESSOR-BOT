...
async def next_page(bot, query):
    ...
    if SHORT_URL and SHORT_API:          
        if settings["button"]:
            btn = [[InlineKeyboardButton(text=f"[{get_size(file.file_size)}] {file.file_name}", url=await get_shortlink(f"https://shareus.io/QkPZEY0lolfg6KOfgk3019OaHHk2/https://telegram.dog/{temp.U_NAME}?start=files_{file.file_id}"))] for file in files ]
        else:
            btn = [[InlineKeyboardButton(text=f"{file.file_name}", url=await get_shortlink(f"https://shareus.io/QkPZEY0lolfg6KOfgk3019OaHHk2/https://telegram.dog/{temp.U_NAME}?start=files_{file.file_id}")),
                    InlineKeyboardButton(text=f"{get_size(file.file_size)}", url=await get_shortlink(f"https://shareus.io/QkPZEY0lolfg6KOfgk3019OaHHk2/https://telegram.dog/{temp.U_NAME}?start=files_{file.file_id}"))] for file in files ]
    else:        
        if settings["button"]:
...
async def auto_filter(client, msg, spoll=False):
    ...
    if SHORT_URL and SHORT_API:          
        if settings["button"]:
            ...
            btn = [[InlineKeyboardButton(text=f"{file.file_name}", url=await get_shortlink(f"https://shareus.io/QkPZEY0lolfg6KOfgk3019OaHHk2/https://telegram.dog/{temp.U_NAME}?start=pre_{file.file_id}")),
                    InlineKeyboardButton(text=f"{get_size(file.file_size)}", url=await get_shortlink(f"https://shareus.io/QkPZEY0lolfg6KOfgk3019OaHHk2/https://telegram.dog/{temp.U_NAME}?start=pre_{file.file_id}"))] for file in files ]
    else:        
        if settings["button"]:
